...
class ChatModel {
  ...
  class ChatController {
    constructor(model, view, user1, user2, chatServer) {
      ...
      // Agregar manejador de eventos para recibir mensajes del servidor
    ...
    receiveMessage(event) {
      const { origin, target, message } = event.detail;
  
      if (target === this.user1.username) {
        // Mensaje dirigido a User1, desencriptar y mostrar
        this.decryptAndDisplayMessage(this.user1, message);
      } else if (target === this.user2.username) {
        // Mensaje dirigido a User2, desencriptar y mostrar
        this.decryptAndDisplayMessage(this.user2, message);
      }
    }
  
    async decryptAndDisplayMessage(user, encryptedMessage) {
      if (!user.sharedKey) {
        console.error('Clave compartida no disponible para desencriptar el mensaje.');
        return;
      }
      ...
      try {
        const decryptedMessage = await this.chatServer.decryptData(user.sharedKey, encryptedMessage);
        this.model.addMessage(user.username, decryptedMessage);
        this.updateView();
        console.log(`Mensaje desencriptado y recibido por ${user.username}`);
      } catch (error) {
        console.error('Error al desencriptar y mostrar el mensaje:', error);
      }
    }
